...
void DeleteEntryByTime(HWND hwnd) {
    ...
    if (strlen(target) == 0) {
        MessageBox(hwnd, "Enter a timestamp or 'all'!", "Error", MB_OK | MB_ICONERROR);
        return;
    }
    ...
    if (f) {
        fputs(buffer, f);
        fclose(f);
        MessageBox(hwnd, "Entry deleted.", "Diary", MB_OK);
    }
}
...
/* Diary buttons - compact layout */
/* Diary buttons - compact layout */
CreateWindow("BUTTON", "Add", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
             20, 20, 70, 28, hwnd, (HMENU)IDC_ADD, hInstance, NULL);

CreateWindow("BUTTON", "View", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
             95, 20, 70, 28, hwnd, (HMENU)IDC_VIEW, hInstance, NULL);

CreateWindow("BUTTON", "Edit", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
             170, 20, 70, 28, hwnd, (HMENU)701, hInstance, NULL);

/* Edit bar beside Edit button */
CreateWindow("EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
             245, 20, 110, 25, hwnd, (HMENU)700, hInstance, NULL);

SendMessage(GetDlgItem(hwnd, 700), EM_SETLIMITTEXT, 64, 0);          // Edit bar

CreateWindow("BUTTON", "Del", WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
             360, 20, 70, 28, hwnd, (HMENU)IDC_OK, hInstance, NULL);
SendMessage(GetDlgItem(hwnd, IDC_TIMESTAMP), EM_SETLIMITTEXT, 64, 0); // Delete bar

/* Timestamp input bar - slightly smaller */
CreateWindow("EDIT", "", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL,
             435, 20, 110, 25, hwnd, (HMENU)IDC_TIMESTAMP, hInstance, NULL);
...
case 701: { // Edit Entry button
    char target[64];
    GetWindowText(GetDlgItem(hwnd, 700), target, sizeof(target));

    if (strlen(target) == 0) {
        MessageBox(hwnd, "Enter a date/time to edit!", "Error", MB_OK | MB_ICONERROR);
        break;
    }

    // Ask for confirmation before overwriting
    int result = MessageBox(hwnd,
        "Do you want to overwrite this entire entry with the text from the main entry box?",
        "Confirm Edit", MB_YESNO | MB_ICONQUESTION);

    if (result != IDYES) {
        break; // user cancelled
    }

    // Get new text from the main entry box
    char newText[1024];
    GetWindowText(GetDlgItem(hwnd, IDC_ENTRYBOX), newText, sizeof(newText));
    if (strlen(newText) == 0) {
        MessageBox(hwnd, "Entry box is empty! Type new text first.", "Error", MB_OK | MB_ICONERROR);
        break;
    }

    // Load diary file and rebuild with replacement
    FILE *f = fopen("diary.txt", "r");
    if (!f) { MessageBox(hwnd, "No diary file found.", "Error", MB_OK | MB_ICONERROR); break; }
    ...
    while (fgets(line, sizeof(line), f)) {
        if (!editing && line[0] == '[') {
            char entryTime[64];
            sscanf(line, "[%63[^]]]", entryTime); // extract timestamp
            if (strcmp(entryTime, target) == 0) {
                // Replace the entire entry with new text
                snprintf(line, sizeof(line), "[%s] %s\r\n", target, newText);
                strncat(buffer, line, sizeof(buffer) - strlen(buffer) - 1);
                editing = 1;
                continue; // skip old entry lines
            }
        }
        if (editing && line[0] == '[') {
            editing = 0; // next entry begins, stop skipping
        }
        if (!editing) {
            strncat(buffer, line, sizeof(buffer) - strlen(buffer) - 1);
        }
    ...
    if (f) {
        fputs(buffer, f);
        fclose(f);
        MessageBox(hwnd, "Entry updated.", "Diary", MB_OK);
    }
    break;
}
